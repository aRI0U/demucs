...
def main(opts=None):
    ...
    for track in args.tracks:
        ...
        out_dir = out / track.stem
        if out_dir.exists():
            print(f"{track} has already been separated. Skipping...")
            continue
        ...
        if args.stem is None:
            for name, source in res.items():
                ...
                if args.sr is not None:
                    print(source, source.shape, source.dtype, args.sr, kwargs["samplerate"])
                    source = source.mean(dim=0).cpu().numpy()
                    source = soxr.resample(source, kwargs["samplerate"], args.sr)
                    source = (source * 32767.).astype(np.int16)
                    wavfile.write(str(stem), args.sr, source[:, np.newaxis])
                else:
